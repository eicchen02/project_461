...
def main():
    ...
    response = requests.get("http://127.0.0.1:8080/package/0", headers=header)
    if(response.status_code == 200):
    ...
    #! Testing (Package By ID Update)

    #TODO Testing (Package By ID Rate)
    response = requests.get("http://127.0.0.1:8080/package/0/rate", headers=header)
    if(response.status_code == 200):
